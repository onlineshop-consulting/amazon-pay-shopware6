...
Component.register('swag-amazon-pay-order-tab', {
    ...
    inject: ['SwagAmazonPayOrderService', 'repositoryFactory'],
    ...
    methods: {
        ...
        loadTransactionDetails(refreshData = false) {
            ...
            return this.loadTransaction(transactionId).then((transaction) => {
                ...
                return this.loadPaymentDetails(
                    chargePermissionId,
                    refreshData
                ).then((response) => {
                    this.isLoading = false;
        ...
        backgroundLoadPaymentHistory(chargePermissionId) {
            this.loadPaymentDetails(chargePermissionId, false).then(
                (response) => {
                    if (!response.chargePermission) {
                        return;
                    }
        ...
        loadPaymentDetails(chargePermissionId, refreshData = false) {
            return this.SwagAmazonPayOrderService.fetchPaymentDetails(
                chargePermissionId,
                refreshData
            );
        },
        ...
        displayErrorMessage(apiError) {
            let errorMessage = this.$tc(
                'swag-amazon-pay-order.general.errors.incompletePaymentProcess'
            );
            if (
                apiError &&
                apiError.response &&
