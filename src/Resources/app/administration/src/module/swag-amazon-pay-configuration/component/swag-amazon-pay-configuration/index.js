...
Component.register('swag-amazon-pay-configuration', {
    ...
    computed: {
            ...
            return [
                {
                    value: 'EUR',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.ledgerCurrency.options.EUR'
                    ),
                },
                {
                    value: 'GBP',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.ledgerCurrency.options.GBP'
                    ),
                },
                {
                    value: 'USD',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.ledgerCurrency.options.USD'
                    ),
                },
            ];
        },
            ...
            return [
                {
                    value: 'immediately',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.authMode.options.immediately'
                    ),
                },
                {
                    value: 'canHandlePending',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.authMode.options.canHandlePending'
                    ),
                },
            ];
        },
            ...
            return [
                {
                    value: 'direct',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.chargeMode.options.direct'
                    ),
                },
                {
                    value: 'shipped',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.chargeMode.options.shipped'
                    ),
                },
                {
                    value: 'manually',
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.chargeMode.options.manually'
                    ),
                },
            ];
        },
            ...
            return [
                {
                    value: 'basic',
                    name: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.loggingMode.options.basic'
                    ),
                },
                {
                    value: 'advanced',
                    name: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.loggingMode.options.advanced'
                    ),
                },
            ];
        },
        ...
        buttonColorOptions() {
            return [
                {
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.buttonColors.gold'
                    ),
                    value: 'Gold',
                },
                {
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.buttonColors.lightGray'
                    ),
                    value: 'LightGray',
                },
                {
                    label: this.$tc(
                        'swag-amazon-pay-configuration.configForm.fields.buttonColors.darkGray'
                    ),
                    value: 'DarkGray',
                },
            ];
    ...
    methods: {
        ...
        loadConfig() {
            ...
            if (this.hasSalesChannelSelect === null) {
                this.systemConfigApiService
                    .getValues(this.configDomain, null)
                    .then((config) => {
            ...
            this.systemConfigApiService
                .getValues(this.configDomain, this.salesChannelId)
                .then((config) => {
                    // Remove domain and config prefix to get variable names on root level
                    Object.keys(config).forEach((key) => {
                        const configActualKey = key.replace(
                            `${this.configDomain}.`,
                            ''
                        );
        ...
        getPrivateKeyPlaceholder() {
            ...
            return this.$tc(
                'swag-amazon-pay-configuration.configForm.privateKeyEmpty'
            );
        },
            ...
            this.SwagAmazonPayConfigService.validateCredentials(credentials)
                .then((response) => {
                    if (response.success) {
                        this.createNotificationSuccess({
                            title: this.$tc(
                                'swag-amazon-pay-configuration.notification.inspectConnectionTitle'
                            ),
                ...
                .catch(() => {
                    this.createNotificationError({
                        ...
                        message: this.$tc(
                            'swag-amazon-pay-configuration.exception.genericError'
                        ),
                    });
                })
                .finally(() => {
        ...
        onSetAmazonPayAsDefaultPaymentMethod() {
            ...
            this.SwagAmazonPayConfigService.salesChannelDefault(
                this.salesChannelId
            ).then(() => {
                window.setTimeout(() => {
