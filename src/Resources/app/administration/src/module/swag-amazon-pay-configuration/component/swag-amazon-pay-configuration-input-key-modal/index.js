...
Component.register('swag-amazon-pay-configuration-input-key-modal', {
    ...
    methods: {
        ...
        onConfirm() {
            if (
                this.privateKey.includes('BEGIN RSA PRIVATE KEY') === false &&
                this.privateKey.includes('BEGIN PRIVATE KEY') === false
            ) {
                this.createNotificationWarning({
                    title: this.$tc(
                        'swag-amazon-pay-configuration.inputKeyModal.notification.warning'
                    ),
                    message: this.$tc(
                        'swag-amazon-pay-configuration.inputKeyModal.notification.invalidKeyFormat'
                    ),
                });
