...
Component.register('swag-amazon-pay-settings', {
    ...
    inject: ['SwagAmazonPayConfigService', 'systemConfigApiService', 'acl'],
    ...
    methods: {
        ...
        async onSave() {
            ...
            if (this.salesChannelId !== null) {
                if (this.noInheritance !== null) {
                    await this.systemConfigApiService.saveValues(
                        {
                            'SwagAmazonPay.settings.noInheritance':
                                this.noInheritance,
                        },
                        null
                    );
            ...
            if (!this.config.inheritFromDefault) {
                Object.keys(this.config).forEach((key) => {
                    const configActualKey = this.configDomain
                        .concat('.')
                        .concat(key);
                ...
                const key = this.configDomain
                    .concat('.')
                    .concat('inheritFromDefault');
            ...
            this.systemConfigApiService
                .saveValues(configToBeSaved, this.salesChannelId)
                .then(() => {
                    this.createNotificationSuccess({
                        title: this.$tc('global.default.success'),
                        message: this.$tc(
                            'swag-amazon-pay-configuration.notification.configSaveSuccessMessage'
                        ),
                    });
        ...
        onExportConfig() {
            ...
            this.SwagAmazonPayConfigService.exportConfig()
                .then((response) => {
                ...
                .catch(() => {
                    this.createNotificationError({
                        title: this.$tc(
                            'swag-amazon-pay-configuration.exception.exportConfig.title'
                        ),
                        message: this.$tc(
                            'swag-amazon-pay-configuration.exception.exportConfig.message'
                        ),
                    });
                })
                .finally(() => {
