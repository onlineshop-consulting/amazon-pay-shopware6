...
export default class AmazonPayButtonProductDetailPlugin extends Plugin {
    ...
    init() {
        this.buttonPlugin =
            window.PluginManager.getPluginInstances('AmazonPayButton')[0];
        this.buttonPlugin.beforeInitCheckout =
            this._getAddProductPromise.bind(this);
    }
    ...
    _getAddProductPromise() {
        this.$emitter.publish(
            'swagAmazonPayProductDetail_beforeAddProductToCart'
        );
                ...
                .then((_response) => {
                    {
                        resolve();
                        this.$emitter.publish(
                            'swagAmazonPayProductDetail_afterAddProductToCart'
                        );
                    }
                });
        });
