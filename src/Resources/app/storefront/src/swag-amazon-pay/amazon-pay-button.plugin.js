...
export default class AmazonPayButtonPlugin extends Plugin {
    ...
    onInsecureConnectionButtonInteraction() {
        document
            .querySelectorAll('.swag-amazon-pay-button-container')
            .forEach((element) => {
                element.style.visibility = 'hidden';
            });

        document
            .querySelectorAll('.swag-amazon-pay-button-error')
            .forEach((element) => {
    ...
    _initializeAmazonScript() {
        ...
        const scriptElementId =
            'swag-amazon-pay-js-' + this.options.buttonConfig.merchantId;
        ...
        this.scriptElement.addEventListener(
            'load',
            this.executeAmazonPayOnLoadCallbacks.bind(this),
            false
        );
    ...
    _initializeEventHandler() {
        ...
        document
            .querySelectorAll('.swag-amazon-pay-button-tooltip-overlay')
            .forEach((element) => {
                if (element.classList.contains('connection-is-insecure')) {
                    element.addEventListener(
                        'click',
                        me.onInsecureConnectionButtonInteraction.bind(me)
                    );
                }
            });
    }
    ...
    renderButton() {
        const amazonPayButton = this.getAmazonPayObject().renderButton(
            `#${this.el.id}`,
            this.options.buttonConfig
        );
    ...
    initCheckout(button) {
        ...
        button.initCheckout({
            createCheckoutSessionConfig:
                this.options.createCheckoutSessionConfig,
        });
    }
    ...
    addListingAddCartAction() {
        this.$emitter.publish(
            'swagAmazonPayButton_beforeListingAddProductToCart',
            { plugin: this }
        );
            ...
            return new Promise((resolve) => {
                fetch(formAction, {
                    ...
                    .then((response) => {
                        ...
                        resolve();
                        this.$emitter.publish(
                            'swagAmazonPayButton_afterListingAddProductToCart',
                            { plugin: this }
                        );
                    })
                    .catch((error) => {
