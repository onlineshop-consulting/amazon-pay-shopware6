...
export default class AmazonLoginButtonPlugin extends Plugin {
    ...
    _initializeAmazonScript() {
        ...
        this.scriptElement.addEventListener(
            'load',
            this._onLoadAmazonPayScript.bind(this),
            false
        );
            ...
            .forEach((element) => {
                if (element.classList.contains('connection-is-insecure')) {
                    element.addEventListener(
                        'click',
                        this.onInsecureConnectionButtonInteraction.bind(this)
                    );
                }
            });
    }
    ...
    _onLoadAmazonPayScript() {
        this.getAmazonPayObject().renderButton(
            `#${this.el.id}`,
            this.options.buttonConfig
        );
        this.scriptElement.removeEventListener(
            'load',
            this._onLoadAmazonPayScript
        );
