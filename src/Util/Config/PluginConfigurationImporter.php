    ...
    public function import(array $config, Context $context, bool $ignoreVersions = false): void
    {
        ...
        foreach ($config as $salesChannelId => $configurationCollection) {
            ...
            if (!$this->hasSalesChannel((string) $salesChannelId, $context)) {
                $this->pluginLogger->info(\sprintf('Skipping configuration import for not existing sales channel %s', $salesChannelId), $loggerContext);

                continue;
            }

            $this->pluginLogger->info(\sprintf('Importing configuration for sales channel %s', $salesChannelId), $loggerContext);
